...
export function Footer() {
  ...
  const link = (path: string, label: string) => (
    <button
      key={path + label}
      onClick={() => go(path)}
      style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "Archivo, sans-serif", fontSize: 13, color: "rgba(245,242,235,0.55)", textAlign: "left", padding: 0, transition: "color 0.15s" }}
  ...
  const col = (heading: string, items: [string, string][]) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "#B79256" }}>{heading}</span>
    ...
    <footer style={{ background: "#101114", borderTop: "1px solid rgba(183,146,86,0.18)" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "72px 32px 40px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 48, marginBottom: 64 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
              <span style={{ fontFamily: "Fraunces, serif", fontSize: 22, fontWeight: 600, color: "#F5F2EB" }}>Fits You</span>
              <FitMark size={8} />
            </div>
            <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: "rgba(245,242,235,0.45)", lineHeight: 1.75, maxWidth: 280, margin: 0 }}>
              Personalized meal kits, supplements, and training — calibrated precisely to you. Delivered across the US.
            </p>
            <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 11, color: "rgba(245,242,235,0.25)", lineHeight: 1.65, maxWidth: 300, margin: 0 }}>
        ...
        <div style={{ borderTop: "1px solid rgba(183,146,86,0.12)", paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 12, color: "rgba(245,242,235,0.28)" }}>© 2024 Fits You · fitsyou.net</span>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "#B79256" }}>Measured. Made for one.</span>
        </div>
      </div>
    </footer>
  );
