    ...
    <div style={{ paddingTop: 64 }}>
      ...
      <section style={{ background: OBSIDIAN, padding: "96px 32px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          <div>
            <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: BRASS }}>Training</span>
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ position: "relative" }}>
                  <div style={{ position: "absolute", bottom: 12, left: 12, background: "rgba(16,17,20,0.82)", padding: "8px 14px", borderRadius: 2, borderLeft: `2px solid ${BRASS}` }}>
                    <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: BRASS }}>SAMPLE — not a real client</span>
                  </div>
                </div>
              </div>
            </FitMark>
      ...
      <section style={{ background: BONE, padding: "128px 32px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ marginBottom: 56 }}>
            <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS }}>Training protocols</span>
            <h2 style={{ fontFamily: "Fraunces, serif", fontSize: 48, fontWeight: 400, color: OBSIDIAN, letterSpacing: "-0.025em", margin: "14px 0 0", lineHeight: 1.05 }}>
            ...
            {PROTOCOLS.map((p, i) => (
              <div key={p.name} style={{ background: i % 2 === 0 ? OBSIDIAN : PINE, padding: "52px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
                  <div>
                    <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS }}>{p.tag}</span>
                    <h3 style={{ fontFamily: "Fraunces, serif", fontSize: 36, fontWeight: 400, color: BONE, margin: "8px 0 0", letterSpacing: "-0.02em" }}>{p.name}</h3>
                  </div>
                </div>
                <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 14, color: "rgba(245,242,235,0.6)", lineHeight: 1.75, margin: "0 0 28px" }}>{p.desc}</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingTop: 20, borderTop: "1px solid rgba(183,146,86,0.14)" }}>
                  {[["Cycle", p.weeks], ["Frequency", p.sessions], ["Equipment", p.equipment]].map(([k, v]) => (
                    <div key={k} style={{ display: "flex", gap: 12 }}>
                      <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: BRASS, minWidth: 80 }}>{k}</span>
                      <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: "rgba(245,242,235,0.65)" }}>{v}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section style={{ background: OBSIDIAN, padding: "96px 32px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 96, alignItems: "center" }}>
          <div>
            <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS }}>The sync</span>
            <h2 style={{ fontFamily: "Fraunces, serif", fontSize: 48, fontWeight: 400, color: BONE, letterSpacing: "-0.025em", margin: "16px 0 24px", lineHeight: 1.05 }}>
              Training and nutrition<br /><em style={{ fontStyle: "italic", color: BRASS }}>in one system.</em>
            </h2>
            <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 15, color: "rgba(245,242,235,0.6)", lineHeight: 1.78, margin: "0 0 32px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {[
              ...
              <div key={label} style={{ padding: "28px 0", borderBottom: i < arr.length - 1 ? "1px solid rgba(245,242,235,0.07)" : "none", display: "flex", gap: 22 }}>
                <span style={{ fontFamily: "Fraunces, serif", fontSize: 34, fontWeight: 300, color: BRASS, opacity: 0.35, minWidth: 48, lineHeight: 1 }}>{String(i + 1).padStart(2, "0")}</span>
                <div>
                  <div style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, fontWeight: 600, color: BONE, marginBottom: 5 }}>{label}</div>
                  <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: "rgba(245,242,235,0.55)", lineHeight: 1.65, margin: 0 }}>{desc}</p>
                </div>
              </div>
            ))}
          </div>
