...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(16,17,20,0.09)" }}>
      ...
      >
        <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 15, fontWeight: 600, color: OBSIDIAN, lineHeight: 1.4 }}>{q}</span>
        <ChevronDown size={16} color={BRASS} style={{ flexShrink: 0, transform: open ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} />
      </button>
      {open && (
        <div style={{ paddingBottom: 24 }}>
          <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 14, color: ASH, lineHeight: 1.75, margin: 0 }}>{a}</p>
        </div>
      )}
    </div>
  );
    ...
    <div style={{ paddingTop: 64 }}>
      ...
      <section style={{ background: OBSIDIAN, padding: "96px 32px 72px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: BRASS }}>Trust + FAQ</span>
      ...
      <section style={{ background: BONE, padding: "96px 32px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS }}>Our commitments</span>
          <h2 style={{ fontFamily: "Fraunces, serif", fontSize: 44, fontWeight: 400, color: OBSIDIAN, letterSpacing: "-0.025em", margin: "14px 0 56px", lineHeight: 1.05 }}>How we operate.</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 2 }}>
            {[
              ...
              <div key={label} style={{ background: i % 2 === 0 ? OBSIDIAN : "#1A1E22", padding: "40px 36px" }}>
                ...
                <h3 style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, fontWeight: 700, color: BONE, margin: "16px 0 10px", letterSpacing: "0.04em" }}>{label}</h3>
                <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: "rgba(245,242,235,0.55)", lineHeight: 1.72, margin: 0 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          {SECTIONS.map(section => (
            <div key={section.heading} style={{ marginBottom: 56 }}>
              <h2 style={{ fontFamily: "Archivo, sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS, margin: "0 0 4px" }}>{section.heading}</h2>
          ...
          <div style={{ background: "#F0EDE5", border: "1px solid rgba(16,17,20,0.1)", borderRadius: 4, padding: "32px", marginTop: 16 }}>
            <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS, display: "block", marginBottom: 12 }}>Results disclaimer</span>
            <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: ASH, lineHeight: 1.75, margin: 0 }}>
