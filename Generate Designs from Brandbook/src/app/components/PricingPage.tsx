    ...
    <div style={{ paddingTop: 64 }}>
      ...
      <section style={{ background: OBSIDIAN, padding: "96px 32px 72px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: BRASS }}>Pricing</span>
          <h1 style={{ fontFamily: "Fraunces, serif", fontSize: "clamp(44px, 5vw, 72px)", fontWeight: 400, color: BONE, letterSpacing: "-0.03em", margin: "16px 0 20px", lineHeight: 1.0 }}>
            Clear. Confident.<br /><em style={{ fontStyle: "italic", color: BRASS }}>No dark patterns.</em>
          </h1>
          <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 16, color: "rgba(245,242,235,0.6)", lineHeight: 1.75, maxWidth: 480, margin: "0 auto" }}>
      ...
      <section style={{ background: BONE, padding: "96px 32px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
              ...
              >
                ...
                <div style={{ marginBottom: 32 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                    <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: plan.highlight ? BRASS : ASH }}>{plan.name}</span>
                    {plan.highlight && <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: OBSIDIAN, background: BRASS, padding: "4px 10px", borderRadius: 2 }}>Most complete</span>}
                  </div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 10 }}>
                    <span style={{ fontFamily: "Fraunces, serif", fontSize: 52, fontWeight: 400, color: plan.highlight ? BONE : OBSIDIAN, letterSpacing: "-0.025em", lineHeight: 1 }}>{plan.price}</span>
                    <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: plan.highlight ? "rgba(245,242,235,0.5)" : ASH }}>{plan.cadence}</span>
                  </div>
                  <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 14, color: plan.highlight ? "rgba(245,242,235,0.6)" : ASH, lineHeight: 1.65, margin: 0 }}>{plan.desc}</p>
                </div>
                  ...
                  <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 10 }}>
                    {plan.includes.map(item => (
                      <li key={item} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: plan.highlight ? "rgba(245,242,235,0.8)" : OBSIDIAN }}>{item}</span>
                      </li>
                    ))}
                    {plan.excludes.map(item => (
                      <li key={item} style={{ display: "flex", alignItems: "center", gap: 10, opacity: 0.35 }}>
                        ...
                        <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: plan.highlight ? "rgba(245,242,235,0.5)" : ASH, textDecoration: "line-through" }}>{item}</span>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 12, color: ASH, textAlign: "center", marginTop: 24, lineHeight: 1.65 }}>
      ...
      <section style={{ background: OBSIDIAN, padding: "96px 32px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS }}>Questions</span>
          <h2 style={{ fontFamily: "Fraunces, serif", fontSize: 44, fontWeight: 400, color: BONE, letterSpacing: "-0.025em", margin: "14px 0 48px", lineHeight: 1.05 }}>Pricing, plain.</h2>
          <div style={{ display: "flex", flexDirection: "column" }}>
            {FAQ_PRICING.map(([q, a], i, arr) => (
              <div key={q} style={{ padding: "32px 0", borderBottom: i < arr.length - 1 ? "1px solid rgba(245,242,235,0.08)" : "none" }}>
                <h3 style={{ fontFamily: "Archivo, sans-serif", fontSize: 15, fontWeight: 600, color: BONE, margin: "0 0 12px", letterSpacing: "-0.01em" }}>{q}</h3>
                <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 14, color: "rgba(245,242,235,0.55)", lineHeight: 1.72, margin: 0 }}>{a}</p>
              </div>
            ))}
          </div>
        </div>
