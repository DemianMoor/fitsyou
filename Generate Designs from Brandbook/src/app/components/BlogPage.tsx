    ...
    <div style={{ paddingTop: 64 }}>
      ...
      <section style={{ background: OBSIDIAN, padding: "80px 32px 64px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: BRASS }}>The Journal</span>
          <h1 style={{ fontFamily: "Fraunces, serif", fontSize: "clamp(40px, 5vw, 72px)", fontWeight: 400, color: BONE, letterSpacing: "-0.03em", margin: "16px 0 16px", lineHeight: 0.97 }}>
            Nutrition, training,<br /><em style={{ fontStyle: "italic", color: BRASS }}>and the science behind both.</em>
          </h1>
          <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 15, color: "rgba(245,242,235,0.55)", lineHeight: 1.72, maxWidth: 480, margin: 0 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: 480 }}>
            ...
            <div style={{ padding: "64px", display: "flex", flexDirection: "column", justifyContent: "center", background: OBSIDIAN }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
                <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: BRASS }}>{featured.category}</span>
                <span style={{ width: 1, height: 12, background: "rgba(183,146,86,0.3)" }} />
                <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 11, color: "rgba(245,242,235,0.4)" }}>{featured.date} · {featured.readTime} read</span>
              </div>
              <FitMark size={20} padded={false}>
                <div style={{ padding: "8px 12px" }}>
                  <h2 style={{ fontFamily: "Fraunces, serif", fontSize: 32, fontWeight: 400, color: BONE, margin: "0 0 16px", lineHeight: 1.2, letterSpacing: "-0.02em" }}>
                    {featured.title}
                  </h2>
                </div>
              </FitMark>
              <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 15, color: "rgba(245,242,235,0.6)", lineHeight: 1.75, margin: "16px 0 32px" }}>
                {featured.excerpt}
              </p>
              <button
                onClick={() => go("/blog")}
                style={{ background: "none", border: "1px solid rgba(183,146,86,0.4)", color: BRASS, cursor: "pointer", fontFamily: "Archivo, sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", padding: "13px 22px", borderRadius: 4, display: "inline-flex", alignItems: "center", gap: 8, alignSelf: "flex-start", transition: "all 0.15s" }}
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 48, paddingBottom: 20, borderBottom: "1px solid rgba(16,17,20,0.09)" }}>
            {CATEGORIES.map(cat => (
              <button
                ...
                style={{ background: activeCategory === cat ? OBSIDIAN : "transparent", color: activeCategory === cat ? BONE : ASH, border: `1px solid ${activeCategory === cat ? OBSIDIAN : "rgba(16,17,20,0.15)"}`, cursor: "pointer", fontFamily: "Archivo, sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", padding: "8px 16px", borderRadius: 4, transition: "all 0.15s" }}
              >
                {cat}
              </button>
            ))}
              ...
              >
                ...
                <div style={{ padding: "32px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                    <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: BRASS }}>{post.category}</span>
                    <span style={{ width: 1, height: 10, background: "rgba(16,17,20,0.2)" }} />
                    <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 11, color: ASH }}>{post.date} · {post.readTime} read</span>
                  </div>
                  <h3 style={{ fontFamily: "Fraunces, serif", fontSize: 22, fontWeight: 400, color: OBSIDIAN, margin: "0 0 12px", lineHeight: 1.25, letterSpacing: "-0.015em" }}>
                    {post.title}
                  </h3>
                  <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 13, color: ASH, lineHeight: 1.7, margin: "0 0 20px" }}>
                    {post.excerpt}
                  </p>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, color: BRASS }}>
                    <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>Read</span>
                    <ArrowRight size={12} />
                  </div>
                </div>
              </div>
          ...
          {rest.length === 0 && (
            <div style={{ textAlign: "center", padding: "64px 0" }}>
              <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 14, color: ASH }}>No articles in this category yet.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section style={{ background: PINE, padding: "96px 32px", borderTop: "1px solid rgba(183,146,86,0.15)" }}>
        <div style={{ maxWidth: 640, margin: "0 auto", textAlign: "center" }}>
          <span style={{ fontFamily: "Archivo, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS }}>The Journal</span>
          <h2 style={{ fontFamily: "Fraunces, serif", fontSize: 40, fontWeight: 400, color: BONE, letterSpacing: "-0.025em", margin: "14px 0 16px", lineHeight: 1.05 }}>New articles, when they're ready.</h2>
          <p style={{ fontFamily: "Archivo, sans-serif", fontSize: 14, color: "rgba(245,242,235,0.55)", lineHeight: 1.72, margin: "0 0 36px" }}>
          ...
          <div style={{ display: "flex", gap: 8, maxWidth: 440, margin: "0 auto" }}>
            <input
              ...
              style={{ flex: 1, fontFamily: "Archivo, sans-serif", fontSize: 14, color: BONE, background: "rgba(245,242,235,0.08)", border: "1px solid rgba(245,242,235,0.18)", borderRadius: 4, padding: "14px 16px", outline: "none" }}
            />
            <button style={{ background: BRASS, color: OBSIDIAN, border: "none", cursor: "pointer", fontFamily: "Archivo, sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", padding: "14px 22px", borderRadius: 4, whiteSpace: "nowrap", transition: "background 0.15s" }} onMouseEnter={e => { e.currentTarget.style.background = "#C9A866"; }} onMouseLeave={e => { e.currentTarget.style.background = BRASS; }}>
              Subscribe
            </button>
          </div>
        </div>
