...
// Individual supplement products (catalog). Optional — the page's primary
// content is the conceptual stack explainer below; this grid appears only when
// published `supplements` rows exist.
...
// Conceptual stack layers (always shown — explainer, not catalog data).
const STACK = [
...
export function SupplementsView({ products }: { products: SupplementCard[] | null }) {
  ...
  return (
    <div style={{ paddingTop: 64 }}>
      ...
      {/* Product catalog (only when published supplements exist) */}
      {hasProducts && (
        <section style={{ background: OBSIDIAN, padding: "96px 32px 0" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
              {products!.map((p, i) => (
                <div key={p.name} style={{ background: i % 2 === 0 ? "#141719" : "#161B1C", padding: "44px", display: "flex", gap: 24 }}>
                  ...
                  <div>
                    {p.category && <span style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: BRASS }}>{p.category}</span>}
                    <h3 style={{ fontFamily: "var(--font-fraunces), serif", fontSize: 24, fontWeight: 400, color: BONE, margin: "8px 0 10px", letterSpacing: "-0.015em" }}>{p.name}</h3>
                    {p.benefit && (
                      <>
                        <p style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 13, color: "rgba(245,242,235,0.65)", lineHeight: 1.7, margin: "0 0 8px" }}>{p.benefit}</p>
                        {p.requiresDisclaimer && (
                          <p style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 10, color: "rgba(245,242,235,0.3)", lineHeight: 1.55, margin: 0 }}>
                            *This statement has not been evaluated by the FDA. Not intended to diagnose, treat, cure, or prevent any disease.
                          </p>
                        )}
                      </>
                    )}
      ...
      <section style={{ background: OBSIDIAN, padding: "96px 32px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ marginBottom: 56 }}>
            <span style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: BRASS }}>The formulation</span>
            <h2 style={{ fontFamily: "var(--font-fraunces), serif", fontSize: 48, fontWeight: 400, color: BONE, letterSpacing: "-0.025em", margin: "14px 0 0", lineHeight: 1.05 }}>
            ...
            {STACK.map((layer, i) => (
              <div key={layer.name} style={{ background: i % 2 === 0 ? "#141719" : "#161B1C", padding: "48px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
                  <div>
                    <span style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRASS }}>{layer.label}</span>
                    <h3 style={{ fontFamily: "var(--font-fraunces), serif", fontSize: 32, fontWeight: 400, color: BONE, margin: "8px 0 0", letterSpacing: "-0.02em" }}>{layer.name}</h3>
                  </div>
                  <FitMark size={14} />
                </div>
                <ul style={{ listStyle: "none", padding: 0, margin: "0 0 24px", display: "flex", flexDirection: "column", gap: 10 }}>
                  {layer.items.map(item => (
                    <li key={item} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <span style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 14, color: "rgba(245,242,235,0.72)" }}>{item}</span>
                    </li>
                  ))}
                </ul>
                <div style={{ borderTop: "1px solid rgba(183,146,86,0.14)", paddingTop: 16 }}>
                  <span style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 11, color: "rgba(183,146,86,0.65)", letterSpacing: "0.06em" }}>{layer.note}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: BONE, padding: "128px 32px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 96, alignItems: "center" }}>
          <div>
            ...
            <p style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 15, color: ASH, lineHeight: 1.78, margin: "0 0 32px" }}>
            ...
            <p style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 12, color: "rgba(16,17,20,0.4)", lineHeight: 1.65 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {[
              ...
              <div key={label} style={{ padding: "26px 0", borderBottom: i < arr.length - 1 ? "1px solid rgba(16,17,20,0.09)" : "none", display: "flex", gap: 22 }}>
                <span style={{ fontFamily: "var(--font-fraunces), serif", fontSize: 32, fontWeight: 300, color: BRASS, opacity: 0.38, minWidth: 48, lineHeight: 1 }}>{String(i + 1).padStart(2, "0")}</span>
                <div>
                  <div style={{ fontFamily: "var(--font-archivo), sans-serif", fontSize: 13, fontWeight: 600, color: OBSIDIAN, marginBottom: 5 }}>{label}</div>
